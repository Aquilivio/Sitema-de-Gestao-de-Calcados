...
public class TelaDeConsulta {
	...
	//Metodo usado para preencher os dados vindos do banco de dados na JTable
	public void PreencherTabelaDeFiltros()
	{
		...
		try {
			...
			JOptionPane.showMessageDialog(null,"Ocorreu Um Erro Nao Previsto: " + e,"ERRO!",JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	private void initialize() {
		...
		txtSelecioneUmFiltro.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				//Limpa o campo que e preenchido com algum filtro mais especifico digitado pelo usuario
		...
		btnFiltrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//IF's que controlam o fluxo de accoes de escolha de filtros pelo usuario
				//IF verifica se a opcao selecionada e a "Selecione" e mostra uma mensagem informando o usuario para escolher um filtro
				if (opcaoDeFiltro == 0)
				{
					DefaultTableModel tabelaDeCalcados = (DefaultTableModel) tblCalcadosFiltrados.getModel();
					tabelaDeCalcados.setNumRows(0);
					JOptionPane.showMessageDialog(null,"Selecione Um Filtro Primeiro!","ATENCAO - Informacao Invalida!",JOptionPane.WARNING_MESSAGE);
				}
				//IF verifica se a opcao selecionada e a "Todos" e faz a requisicao para o banco de dados com todos os calcados registrados no banco de dados
				else if (opcaoDeFiltro == 1)
				{
					try {
						...
						//tratamento para eventuais erros 
						JOptionPane.showMessageDialog(null,"Ocorreu Um Erro Nao Previsto: " + e1,"ERRO!",JOptionPane.WARNING_MESSAGE);
					}
				}
				//IF verifica se o usuario digitou algum filtro especifico 
				else if (txtFiltroDosCalcados.getText().equals(""))
					JOptionPane.showMessageDialog(null,"Indique Um Filtro Primeiro!","ATENCAO - Informacao Faltando!",JOptionPane.WARNING_MESSAGE);
				//IF verifica se a opcao selecionada e a "Tamanho" e faz a requisicao para o banco de dados com o tamanho especificado por calcados registrados no banco de dados
				...
				//IF verifica se a opcao selecionada e a "Categoria" e faz a requisicao para o banco de dados com a categoria especificada por calcados registrados no banco de dados
				...
				//IF verifica se a opcao selecionada e a "Cor" e faz a requisicao para o banco de dados com a cor especificada por calcados registrados no banco de dados
				...
				//IF verifica se a opcao selecionada e a "Menor Preco" e faz a requisicao para o banco de dados com o menor preco especificado por calcados registrados no banco de dados
				else if (opcaoDeFiltro == 5)
				{
					try {
						...
						//Trata o preco antes de ser usado na requisicao para o banco de dados
						String precoTratado = txtFiltroDosCalcados.getText().toString();
						precoTratado = precoTratado.trim();
						precoTratado = precoTratado.replace("MZN", "");
				...
				//IF verifica se a opcao selecionada e a "Marca" e faz a requisicao para o banco de dados com a marca especificada por calcados registrados no banco de dados
				else
				{
					try {
						//Limpa o ResultSet
